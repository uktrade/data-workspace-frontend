...
docker-test-integration-local:
	...
	docker-compose -f docker-compose-test-local.yml -p data-workspace-test run data-workspace-test bash -c "DJANGO_SETTINGS_MODULE=dataworkspace.settings.integration_tests pytest -x -v -s $$TEST_DIR --disable-warnings"
