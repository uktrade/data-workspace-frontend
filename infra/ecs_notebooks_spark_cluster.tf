resource "aws_ecs_task_definition" "sparkcluster" {
  family                = "${var.prefix}-sparkcluster"
  container_definitions = "${data.template_file.sparkcluster_container_definitions.rendered}"
...
data "external" "sparkcluster_current_tag" {
  program = ["${path.module}/task_definition_tag.sh"]

  query = {
    task_family = "${var.prefix}-sparkcluster"
    container_name = "${local.notebook_container_name}"
  }
}

data "external" "sparkcluster_metrics_current_tag" {
  program = ["${path.module}/task_definition_tag.sh"]
...
data "external" "sparkcluster_s3sync_current_tag" {
  program = ["${path.module}/task_definition_tag.sh"]
...
data "template_file" "sparkcluster_container_definitions" {
  template = "${file("${path.module}/ecs_notebooks_notebook_container_definitions.json")}"

  vars = {
    container_image  = "${aws_ecr_repository.spark_cluster.repository_url}:master"
    container_name   = "${local.notebook_container_name}"
    ...
    home_directory = "/home/sparkcluster"
  }
}
