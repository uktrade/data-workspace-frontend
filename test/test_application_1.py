...
class TestApplication(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_arango_no_permissions(self):
        await flush_database()
        await flush_redis()
    ...
    async def test_arango_team_database_permissions(self):
        ...
        async with session.request(
            "GET",
            "http://testdbapplication-23b40dd9.dataworkspace.test:8000/arango",
            params={"database": "team_test1"},
        ) as response:
            status = response.status
            content = await response.text()

        self.assertEqual(status, 200)
        self.assertEqual('{"data": "rw"}', content)

        async with session.request(
            "GET",
            "http://testdbapplication-23b40dd9.dataworkspace.test:8000/arango",
            params={"database": "team_test2"},
        ) as response:
            status = response.status
            content = await response.text()

        self.assertEqual(status, 200)
        self.assertEqual('{"data": "none"}', content)

    async def test_arango_catalogue_permissions(self):
        session, cleanup_session = client_session()
        self.addAsyncCleanup(cleanup_session)
        ...
        async with session.request(
            "GET",
            "http://testdbapplication-23b40dd9.dataworkspace.test:8000/arango",
            params={"database": "Datasets", "collection": "testcollection1"},
        ) as response:
            status = response.status
            content = await response.text()

        self.assertEqual(status, 200)
        self.assertEqual('{"data": "ro"}', content)

        async with session.request(
            "GET",
            "http://testdbapplication-23b40dd9.dataworkspace.test:8000/arango",
            params={"database": "Datasets", "collection": "testcollection2"},
        ) as response:
            status = response.status
            content = await response.text()
