...
def _parse_postgres_log(log_reader, from_date, to_date):
    ...
    for log_line in log_reader:
        ...
        # Ignore common noise generated by pgadmin
        if log_line['statement'] in settings.PGAUDIT_IGNORE_STATEMENTS:
            continue
...
def _do_sync_tool_query_logs():
    ...
    for log in logs:
        ...
        try:
            db_user = ApplicationInstanceDbUsers.objects.get(
                db=database, db_username=log['user_name']
            )
        except ApplicationInstanceDbUsers.DoesNotExist:
            logger.info(
