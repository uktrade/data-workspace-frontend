...
class RequestingDataWizardView(NamedUrlSessionWizardView, FormPreview):
    ...
    def done(self, form_list, **kwargs):
        requesting_dataset = RequestingDataset.objects.get(name=self.get_cleaned_data_for_step('name'))
    ...
    def process_step(self, form):
        ...
        for field, value in self.get_cleaned_data_for_step('name').items():
            print("PROCESS STEP!!!!")
            if self.steps.current == "summary":
                requesting_dataset = RequestingDataset.objects.create(name=value)
            ...
            # TODO DatasetUsageForm to be sent to restrictions on usage.
                ...
                for form in self.form_list:
                    ...
                    print('stepdata:', self.storage.data["step_data"])

                    for step in self.storage.data["step_data"]:
                        print("STEP:", step)
                        print('name cleaned:', self.get_cleaned_data_for_step('name'))

                        for field, value in self.get_cleaned_data_for_step(step).items():
                            print('cleaned:', self.get_all_cleaned_data)
                            print("FIELD:", field)
                            print('VALUE:', value)
                            ...
                            #     self.storage.data.pop(data)
                            #     print("AFTER", self.storage.data)
                            return HttpResponseRedirect(
                                reverse(
                                    "datasets:find_datasets",
                                )
            ...
            print("HERE", self.get_form_step_data(form))
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.steps.current == "summary":
            ...
            print("STEP DATA", self.storage.data["step_data"])
