...
class ChartBuilderChartManager(models.Manager):
    def _create_chart(self, query_log, user, source=None):
    ...
    def create_from_source(self, source, user, run_query=True):
        ...
        return self._create_chart(new_query_log, user, source)

    def create_from_sql(self, query, user, connection):
        new_query_log = QueryLog.objects.create(
            sql=query,
            run_by_user=user,
        ...
        return self._create_chart(new_query_log, user)
