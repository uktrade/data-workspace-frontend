...
def get_new_exporter_class(format_):
    class_str = dict(settings.EXPLORER_DATA_EXPORTERS)[f"new-{format_}"]
    return import_string(class_str)


class NewBaseExporter:
    name = ''
    content_type = ''
    file_extension = ''
...
class NewCSVExporter(NewBaseExporter):
...
class NewJSONExporter(NewBaseExporter):
...
class NewExcelExporter(NewBaseExporter):
