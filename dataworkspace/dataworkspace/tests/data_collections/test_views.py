...
from dataworkspace.tests.conftest import get_staff_client, get_user_data


def test_collection(client):
    user = factories.UserFactory(is_superuser=True)
    client = get_staff_client(get_user_data("", user))
...
def test_unpublished_raises_404(client):
    ...
    client = get_staff_client(get_user_data("my_database", user))
