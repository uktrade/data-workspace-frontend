...
describe('Homepage dashboard', () => {
  ...
  context('When a user visits the page for the first time', () => {
    ...
    it('should not show the managed data tile', () => {
      cy.wait('@managedData');
      cy.get('article')
        .contains("You're the owner or manager of ")
        .should('not.exist');
    });
  ...
  context('When an exisiting user visits the page', () => {
    ...
    it("should show the 'Managed Data' section with with a figure", () => {
      ...
      cy.findByRole('link', { name: 'View and manage your data' }).should(
        'have.attr',
        'href',
        '/datasets/?q=&sort=relevance&my_datasets=owned'
      );
      cy.findByRole('link', {
        name: /Learn how to maintain and manage data/i
      }).should(
        'have.attr',
        'href',
    ...
    it("should show the 'Your bookmarks' section with bookmarks", () => {
      ...
      cy.findByTestId('your-bookmarks').within(() => {
